...
[Shared state](/claude-sdk-python/shared-state) is most powerful when the agent's state shows up in
...
transition, or [streamed update](/claude-sdk-python/shared-state/streaming), `useAgent` re-renders this component
...
This is the same two-way channel described in [Shared State](/claude-sdk-python/shared-state). The
...
- [Shared State](/claude-sdk-python/shared-state): the full read/write model and the underlying `useAgent` subscription.
- [State streaming](/claude-sdk-python/shared-state/streaming): stream a tool argument into a state key so the canvas fills in token-by-token.
- [Agent read-only context](/claude-sdk-python/shared-state/agent-readonly): push UI values *to* the agent without letting it write back.
